...
LogitBoost <- function (x, ...)
  UseMethod("LogitBoost")


LogitBoost.formula <- function (formula, data, ..., subset, na.action) {
  ...
  if (xint > 0L)
    x <- x[, -xint, drop = FALSE]
  res <- LogitBoost(x, grouping, ...)
  # res <- lda.default(x, grouping, ...)
  res$terms <- Terms
  cl <- match.call()
  cl[[1L]] <- as.name("LogitBoost")
  res$call <- cl
...
LogitBoost.default <- function(x, y, nIter=ncol(x), ...)
...
predict.LogitBoost = function(object, newdata, type = c("class", "prob"),
                              nIter=NA, ...)
  #   An implementation of the LogitBoost classification algorithm with
  #   decision stumps as weak learners.
  #
...
#   available on the web page http://stat.ethz.ch/~dettling/boosting.html
{
  ...
  if(!missing(newdata))
    ...
    xtest <- model.frame(object)[ , -c(attr(object$terms, "response"))]
  ...
  if (nClass>2) {                     # multi class problem
    ...
    for(iClass in 1:nClass) {
      ...
      prob = predict.LogitBoost(object, xtest, type="prob", nIter=nIter)
  ...
  if (type=="prob") RET=Prob        # request to return raw probabilities
